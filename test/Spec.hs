import Test.Hspec
import Test.QuickCheck (property)

import Lib (sumUpMessages, RemoteMsg(Connect, Ping))

main :: IO ()
main = hspec $ do
  describe "Lib.sumUpMessages" $ do
    it "counts the elements" $ do
      property $ \numbers ->
        let msgs = Ping undefined undefined 0 <$> numbers
        in fromInteger (fst (sumUpMessages msgs)) == length numbers

    it "computes the scalar product of the values" $ do
      let numbers = [(2, 0.8), (5, 0.5), (0, 0.1), (1, 0.6)]
          msgs = uncurry (Ping undefined undefined) <$> numbers
      snd (sumUpMessages msgs) `shouldBe` 5.7

    it "filters for only Pings" $ do
      let msgs = [Ping undefined undefined 0 0.5, Connect undefined]
      fst (sumUpMessages msgs) `shouldBe` 1
